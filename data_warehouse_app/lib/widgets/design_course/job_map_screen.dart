...
class LatLngUnion<double, doube, int> {
  ...
  String toString() {
    ...
    return lat.toString() + "," + long.toString() + ' have ' + nUnion.toString();
  }
}
...
class _JobMapScreenState extends State<JobMapScreen> {
  ...
  void _addSymbol(String iconImage, LatLng point, double iconSize) {
  ...
  void _extractMapInfo() {
    ...
    if (_circleWeNeedButWeNotDeserved != null)
      if (lastZoom != _position.zoom) {
          mapController.updateCircle(_circleWeNeedButWeNotDeserved, CircleOptions(
              geometry: LatLng(widget._currentLocation.latitude,
                  widget._currentLocation.longitude),
              circleRadius: 55 / (pow(2, 11 - _position.zoom)) * _searchRange/sqrt(2),
              circleColor: "#e84118",
              circleOpacity: 0.2
          ));
  ...
  void _getNearJob(double lat, double long, double radius) async {
    ...
    directionAPI.routeBounds(
        LatLng(lat - radius/111.32, long - radius/(40075 * cos( lat - radius/111.32 ) / 360)),
        LatLng(lat + radius/111.32, long + radius/(40075 * cos( lat + radius/111.32 ) / 360))
    );
    ...
    mapController.addCircle(CircleOptions(
      geometry: LatLng(lat, long - radius/(40075 * cos( lat + radius/111.32 ) / 360)),
      circleRadius: 5,
      circleColor: "#00b894",
      // circleOpacity: 0.6
    ));
    ...
    //   _addSymbol('assets/symbols/office-building.png', LatLng(job.lat, job.long), 1 / _position.zoom * 3);
    // });
    ...
    for (int i = 0; i < union.length; i++) {
      ...
      unionSymbols.add(SymbolOptions(
        geometry: LatLng(union[i].lat/union[i].nUnion, union[i].long/union[i].nUnion),
